...
3. [ ] Wat is het sentiment (negatief/positief) van de woorden die het meest worden geretweet/gefavorite?
...
tweets_fy_words <- tweets_first_year %>%
  ...
  rename(word = words)
# Add afinn lecxicon and scores with, words most retweets
most_rt_words <- inner_join(tweets_fy_words, get_sentiments("afinn")) %>%
  arrange(desc(retweet_count)) %>%
  select(c(word, score, retweet_count, favorite_count, ID))
# Html widget interactive table in rmarkdown report
datatable(head(most_rt_words, n = nrow(most_rt_words)), options = list(pageLength = 10))
```

**Berekenen MEDIAN rt's van woorden en dan pas sorteren!**
